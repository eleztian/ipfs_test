...
func NewRequest(ctx context.Context, url, command string, args ...string) *Request {
	...
	return &Request{
		ApiBase: url + "/api/" + IPFS_VERSION_V0,
		Command: command,
		Args:    args,
		Opts:    opts,
...
func (r *Request) Send(c *http.Client) (*Response, error) {
	...
	if resp.StatusCode >= http.StatusBadRequest {
		...
		case contentType == "text/plain":
			...
			if err != nil {
				fmt.Fprintln(os.Stderr, "ipfs-shell: warning! response error: %s\n", err)
			}
			e.Message = string(out)
		case contentType == "application/json":
			if err = json.NewDecoder(resp.Body).Decode(e); err != nil {
				fmt.Fprintln(os.Stderr, "ipfs-shell:warning! response unmarshall error: %s\n", err)
			}
		default:
			fmt.Fprintln(os.Stderr, "ipfs-shell:warning! unhandled response encoding: %s", contentType)
			out, err := ioutil.ReadAll(resp.Body)
			if err != nil {
